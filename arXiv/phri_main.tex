...
terms and normalizes the task inertia, leaving a linear double-integrator
backbone whose state-transition matrix is configuration-independent, with all
robot dependence isolated in the input matrix.
...
workspace-wide stability, and sampled-data joint-limit safety follow with
...
fixed transition matrix, it offers an analytic interaction-dynamics prior for
model-based physical AI, on which a learned model need only capture the residual
uncertainty of human intent, contact, and environment.
\end{abstract}

\begin{IEEEkeywords}
Linear double integrator, impedance control, model predictive control,
...
error dynamics}---the closed-loop relation among commanded motion, contact
force, actuator and joint limits, and safety filters---the quantity that is
explicitly modeled, predicted, and optimized. We call this perspective
\emph{interaction dynamics} (formalized in Definition~\ref{def:id}), and this
paper develops one concrete, offset-free predictive realization of it. We use
the term in a deliberately specific modeling sense---the augmented
error-and-force dynamics at the contact port taken as the object of prediction
...
the configuration) rather than in the underlying mechanics. Within that scope,
and rather than viewing robot control as predicting robot motion and reacting to
contact, we advocate predicting the interaction evolution directly.
...
from LPV predictive control~\cite{cao2005minmax}. Three consequences follow:
interaction shaping, disturbance rejection, and constraints live in one convex
program; the persistent contact force is predicted and cancelled rather than
merely reacted to; and, because $A_d$ is configuration-independent, the
free-response matrix is precomputed once, pointing toward interaction models
that transfer across configurations.
...
interaction error at the contact port---the augmented error state
$x_{k+1}=A_dx_k+B_d(\rho_k)u_k+E\hat d_k$ (Definition~\ref{def:id})---rather
...
feedforward cancellation makes the transition matrix $A_d$ \emph{constant}
across all configurations, with the entire robot dependence isolated in the
input matrix $B_d(\rho_k)$ (the model as a whole is thus \emph{not}
configuration-independent---only its transition matrix is). This lets the
free-response matrix be precomputed once and the online problem stay a fixed
30-variable QP at 100\,Hz at every configuration, and it reduces closed-loop
stability to a finite vertex LMI that certifies exponential stability of the
interaction dynamics across the entire singularity-free workspace
(Theorem~\ref{thm:workspace}) rather than only at a frozen
configuration---the structural property that points toward interaction models
transferable across configurations.
...
This connects the present work to the broader agenda of \emph{model-based
physical AI}. Recent embodied systems couple a perception--language stack to a
robot through a learned world model $x_{k+1}=f(x_k,u_k)$ that is rolled out for
prediction and planning; the hardest part of that model is contact, precisely
because the underlying dynamics $M(q)\ddot q+C\dot q+G=\tau+J^\top\mathcal F_h$
are nonlinear and configuration-dependent, which makes purely learned contact
models data-hungry. The interaction-dynamics backbone offers an \emph{analytic}
alternative for the physical layer: operational-space cancellation reduces the
contact dynamics to a linear model whose transition matrix is fixed and
configuration-independent and whose robot dependence is a single known input
matrix. A
model-based agent built on this backbone need not re-learn robot dynamics; the
residual left to a learned component is only the genuinely uncertain
part---human intent, contact force, and environment---which enters through the
disturbance channel the estimator already tracks. We do not claim a physical-AI
system here; rather, we position an interaction-dynamics representation with a
fixed, configuration-independent transition matrix as an analytic dynamics
...
force and joint friction appear as an aggregated Cartesian acceleration
disturbance $d(t)$, modelled as a random walk:
\begin{equation}\label{eq:distmodel}
  \dot{d}=w(t),\quad w\sim\mathcal{N}(0,Q_d)
\end{equation}
...
the configuration-space dynamics $M(q)\ddot q + C(q,\dot q)\dot q + G(q) = \tau
+ J^\top(q)\mathcal{F}_h$ as the object to be predicted and regulated,
...
% ----------------------------------------------------------
\section{Linear Double-Integrator Backbone and Finite-Horizon Realization}
...
After applying \eqref{eq:tff}, the residual error dynamics satisfy:
\begin{equation}\label{eq:errordyn}
  \ddot{e} = -\Laminv\,\Fmpc + d(t)
\end{equation}
where $e=p_d-p$ and the aggregated disturbance is:
\begin{align}\label{eq:dist}
  d(t) &= \underbrace{-\Laminv\,\bar{J}_v^\top J^\top\mathcal{F}_h}_{%
...
\emph{modelled} as the disturbance $d(t)$, which the augmented Kalman state
...
scheduling variable $\rho_k=q_k$, discretization gives:
\begin{equation}\label{eq:lpv}
  x_e(k+1) = \underbrace{\begin{bmatrix}I_3&\Delta t I_3\\0&I_3\end{bmatrix}}_{A_d\;\text{(constant)}}
              x_e(k)
            + \underbrace{\begin{bmatrix}0\\-\Laminv\Delta t\end{bmatrix}}_{B_d(\rho_k)}
              \Fmpc(k)
\end{equation}
...
$x_{k+1}=Ax_k+Bu_k+Ed_k$ of Definition~\ref{def:id}; offset-free tracking,
...
\textbf{Discretization note on $B_d$.}
The input matrix in \eqref{eq:lpv} uses Forward-Euler approximation.
The exact ZOH integral gives
$B_d^{\text{ZOH}}=[-\tfrac{\Delta t^2}{2}\Laminv;\;-\Delta t\Laminv]^\top$;
the neglected top-block is $O(\Delta t^2)\approx2.5\times10^{-4}$\,m/N
at 100\,Hz, appearing as a known constant component of $\dhat$ (which
does not bias the offset-free guarantee). Substituting the exact ZOH
block removes the artifact without breaking the constant-$A_d$ structure
--- only $B_d(\rho_k)$ rescales.
Both $\Gamma$ and the Kalman augmented state \eqref{eq:augstate} must
use the same $B_d$ formula to avoid bias in $\dhat$; our
implementation uses the Forward-Euler $B_d$ consistently in both.

\begin{remark}[Controllability and Detectability]\label{rem:feasibility}
The LPV system \eqref{eq:lpv} is uniformly controllable across the
singularity-free workspace.
Since $\Lambda(q)=(J_vM^{-1}J_v^\top)^{-1}$ is SPD for all $q$ away
from kinematic singularities, $B_d(\rho_k)$ has full column rank~3.
The controllability matrix $\mathcal{C}=[B_d\mid A_dB_d]$ has rank~6,
since:
\begin{equation*}
  A_dB_d = \begin{bmatrix}I&\Delta t I\\0&I\end{bmatrix}
            \begin{bmatrix}0\\-\Laminv\Delta t\end{bmatrix}
          = \begin{bmatrix}-\Laminv\Delta t^2\\-\Laminv\Delta t\end{bmatrix}
\end{equation*}
and $\Laminv$ is full rank.
If the stage penalty satisfies $Q\succeq0$ and $(Q^{1/2},A_d)$ is
detectable, then the discrete-time LQR/DARE solution exists at every
fixed configuration and the corresponding gain
$K_\infty=(R+B_d^\top P_\infty B_d)^{-1}B_d^\top P_\infty A_d$ is
well-defined. For the augmented estimator, observability of
$[e;\dot e]$ together with full column rank of $B_d$ makes the constant
disturbance state detectable: if a mode with eigenvalue $\lambda$ is
unobservable from $C=[I_6\;0]$, then its $x_e$ component is zero; the
augmented dynamics then require $B_d d=0$, hence $d=0$.
\end{remark}

\textit{Implementation note (regularization).}
In code the operational-space inverse is formed as
$\Lambda^{-1}(q)=J_vM^{-1}J_v^\top+\sigma I$ with $\sigma=10^{-6}$ for
numerical conditioning. This Tikhonov term is negligible in the
well-conditioned workspace interior but mildly damps the estimated disturbance
(and the realized stiffness) along directions that become ill-conditioned as a
kinematic singularity is approached; operation is restricted to the
singularity-free workspace, where its effect stays below the measurement-noise
floor.

The receding-horizon QP with $N=10$ steps, $3N=30$ decision variables
$U=[\Fmpc(0);\ldots;\Fmpc(N-1)]$, is:
\begin{subequations}\label{eq:QP}
\begin{align}
  \min_U\;& \tfrac{1}{2}U^\top HU+h^\top U \label{eq:QP_cost}\\
  \text{s.t.}\;&
  -\tau_{\max}\leq
  \tau_{\text{base}}(0)+J_v^\top(q_0)\Fmpc(0)
  \leq\tau_{\max}, \label{eq:QP_torque}\\
  & -F_{\max}\leq\Fmpc(k)\leq F_{\max},
  \quad k=0,\ldots,N-1. \label{eq:QP_force}
\end{align}
\end{subequations}
with $H=\Gamma^\top\bar{Q}\Gamma+\bar{R}$, stage cost
...
is affine in $\Fmpc(0)$ --- a convex per-joint polytope solved by OSQP at the
...
$\tau_{\text{base}}(0)$ alone exceed $\tau_{\max}$, the constraint is softened
with a penalized slack $s\geq0$ so the QP stays feasible and returns the
minimum-violation command.
...
\begin{remark}[Prescribed vs.\ realized gains]
...
$\Fmpc(k)=-K_\infty(\rho_k)x_e(k)$. By Remark~\ref{rem:feasibility} the
...
\subsection{Kalman Disturbance Augmentation for Offset-Free Tracking}
...
random walk $d(k{+}1)=d(k)+w(k)$:
\begin{equation}\label{eq:augstate}
\begin{aligned}
  \begin{bmatrix}x_e(k+1)\\d(k+1)\end{bmatrix}
  &= \underbrace{\begin{bmatrix}A_d&B_d(\rho_k)\\0&I_3\end{bmatrix}}_{A_{\text{aug}}}
    \begin{bmatrix}x_e(k)\\d(k)\end{bmatrix}\\[2pt]
  &\quad + \begin{bmatrix}B_d(\rho_k)\\0\end{bmatrix}\Fmpc(k)
  + \begin{bmatrix}0\\w(k)\end{bmatrix},
\end{aligned}
\end{equation}
...
configuration, with constant disturbance $d(k)=d_\infty$. Assume:
(i) the augmented estimator is detectable and its estimation error is
asymptotically stable, so $\hat d(k)\to d_\infty$; (ii) the receding
...
$\lim_{k\to\infty}e(k)=0$. If $d(k)\to d_\infty$ asymptotically, the
...
same equilibrium. Hence $x_e(k)\to0$ and therefore $e(k)\to0$.
...
constant. Theorem~\ref{thm:workspace} removes this caveat, certifying that the
closed-loop interaction dynamics are exponentially stable along \emph{any}
configuration trajectory in the workspace; the moving-reference experiments in
Section~\ref{sec:experiments} then illustrate that guarantee, with the
offset-free property holding at each sustained-contact configuration the
trajectory visits.
\end{remark}

\begin{theorem}[Workspace Stability of the Interaction Dynamics]\label{thm:workspace}
Consider the LPV interaction dynamics
$x_e(k+1)=A_d x_e(k)+B_d(\rho_k)\Fmpc(k)$ with constant $A_d$ and
$B_d(\rho)=[0;\,-\Laminv(\rho)\Delta t]$, and suppose $\Laminv(q)$ lies in a
compact polytope over the singularity-free workspace,
...
with vertex input matrices $B_v=[0;\,-L_v\Delta t]$. If there exist
...
\end{equation}
then the fixed feedback $\Fmpc=Kx_e$, $K=YQ^{-1}$, makes $V(x)=x^\top Px$,
$P=Q^{-1}$, a \emph{common} quadratic Lyapunov function
($V(x_e(k{+}1))\le\gamma V(x_e(k))$, $\gamma<1$) for \emph{every} admissible
configuration trajectory $\{\rho_k\}$; the interaction error is thus
exponentially stable uniformly over the workspace, not only at a frozen
configuration.
\end{theorem}
...
how $\rho_k$ varies. Composing with the detectable integrating-disturbance
estimator of Theorem~\ref{thm:zss} makes the tracking error offset-free at each
constant-contact equilibrium the trajectory visits.
\end{proof}

This is the workspace-level counterpart of the per-configuration DARE
...
interaction-dynamics representation across the entire workspace. As in
Theorem~\ref{thm:zss}, the certificate is for the predictive-plus-estimator core
with the input constraint inactive; the conditional joint-limit and passivity
filters (Theorem~\ref{thm:invariance}, Proposition~\ref{prop:tank}) are
enforced separately.

\begin{remark}[Numerical certificate]
For the FR3, sampling $\Laminv(q)$ along both benchmarks (4600 samples) gives
$\operatorname{eig}\Laminv\in[0.059,0.360]\,\text{kg}^{-1}$ (task inertia
$2.8$--$16.9$\,kg). Solving~\eqref{eq:lmi} over the resulting 64-vertex box
($\Delta t=10$\,ms) is feasible, returning a common $P$ with
$\operatorname{cond}P\approx3.7$, closed-loop spectral radius $\le0.996$ at every
vertex, and $\max\operatorname{eig}(A_\text{cl}^\top P A_\text{cl}-P)=-1.5\times10^{-3}$.
The guaranteed common-$P$ rate is conservative---the realized MPC converges much
faster---but it establishes the workspace-stability claim rigorously rather than
empirically.
\end{remark}
...
$\dhat(k+i\mid k)=\dhat(k\mid k)$ --- a flat extrapolation that is exact only
...
$\|\dot{d}(t)\|_2\le L_d$, and let $\|d(k)-\dhat(k\mid k)\|_2\le e_K$ be the
...
\begin{equation}\label{eq:predbound}
  \|d(k+i)-\dhat(k+i\mid k)\|_2 \;\le\; e_K + L_d\,i\,\Delta t,
  \qquad i=0,\dots,N-1,
\end{equation}
...
follows by the triangle inequality on $d(k+i)=d(k)+\int\dot{d}\,d\tau$. The
...
\eqref{eq:predbound} benign. (i)~\emph{Receding horizon:} only $\Fmpc(0)$ is
applied and the QP is re-solved every $\Delta t$ with a fresh
$\dhat(k\mid k)$, so the loose late-horizon predictions never reach the plant
...
disturbance-prediction RMS}, $\varepsilon_N=\mathrm{RMS}_k\,\|d(k)-\dhat(k\mid
...
This runs at 1\,kHz and is decoupled from the translational QP through
the null-space projector in \eqref{eq:tau}.

\subsection{Null-Space Joint-Limit Safety and Invariance}
...
\subsection{Energy-Tank Passivity Layer for Co-Manipulation}
...
The certificate is scoped to the translational task channel; the excluded
channels are non-generative by construction, so leaving them outside the
tank does not compromise it. The orientation channel is a passive PD
($K_{\text{rot}},D_{\text{rot}}>0$), the null-space torque only
redistributes energy within the redundant subspace, and the feedforward
gravity/Coriolis compensation is workless at steady state. A full 6-DOF
port certificate that also bounds the rotational channel is a
straightforward extension (a second tank on $(\tau_{\text{orient}},\omega)$)
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{figures/mpc_comparison_results.png}
...
Key findings:
(1)~\emph{Prediction is the dominant contributor here:} C4 reduces
contact-window RMS from 41.1\,mm to 2.2\,mm ($19\times$) over C1,
validating the constant-$A_d$ QP.
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{figures/trajectory_boundary.png}
...
\emph{time-varying} human force and directly measure the $N$-step
...
Because the true disturbance $d(k)$ is not directly measurable, we use the
filtered estimate $\dhat(k\mid k)$ as its proxy and the flat prediction
$\dhat(k\mid k{-}N)=\dhat(k{-}N\mid k{-}N)$, so $\varepsilon_N$ measures the
$N$-step self-consistency of the estimate.

\begin{table}[!t]
\caption{Time-Varying Force --- Horizon-Prediction Error and Tracking}
\label{tab:tv}
\renewcommand{\arraystretch}{1.2}
\centering
\setlength{\tabcolsep}{4pt}
...
The measured horizon-extrapolation gap $\varepsilon_N-\varepsilon_1$ tracks the
Section~III-D bound term $L_d N\Delta t$ to within ${\approx}10\%$ across a decade
of disturbance rate (Table~\ref{tab:tv}), confirming the flat-prediction error is
\emph{linear in the force rate}, with a constant-force floor $\approx0.38$\,N at the
$f{=}0$ row --- the process-noise-jitter $e_K$-analogue of~\eqref{eq:predbound}.
Tracking degrades gracefully: sub-millimeter up to $L_d\approx21$\,N/s
--- brisker than typical voluntary guidance --- and ${\approx}1$\,mm at 43\,N/s,
recovering the offset-free property (Theorem~\ref{thm:zss}) as $f\to0$. This
validates the use of the flat random-walk model inside the receding-horizon loop;
...
Prediction (high QP rate) addresses transient peak deflection;
estimation (Kalman) drives steady-state error to zero.
These are orthogonal: neither alone achieves both; their combination
(C7/G7) achieves best-in-class contact behavior in both benchmarks.

\textbf{Constant $A_d$ advantage.}
The feedforward cancellation enables $\Phi$ to be precomputed, the QP
to have exactly 30 decision variables at all configurations and for all
robot models, and warm-started OSQP to converge in under 0.5\,ms in our
simulation runs.

\textbf{Platform generality.}
The controller requires only $M(q)$, $C\dot{q}+G(q)$, and $J_v(q)$,
available from MuJoCo, Pinocchio, KDL, or any robot dynamics library.

% ----------------------------------------------------------
\section{Conclusion}

Classical robot control predicts robot dynamics and treats contact as a
disturbance; this paper argued instead that safe pHRI should be designed by
predicting \emph{interaction dynamics} directly: the closed-loop relation among
commanded motion, human-applied force, actuator limits, joint limits, and
passivity (Definition~\ref{def:id}). This shift makes compliance, tracking,
disturbance rejection, and safety emergent properties of one optimization over
the interaction error rather than four separately designed modules.
The proposed framework makes those dynamics explicit by using
operational-space feedforward cancellation to expose a linear
double-integrator backbone for the translational task.
The nonlinear robot is still represented through measured dynamics, but
its configuration dependence is confined to the input matrix
$B_d(\rho_k)$, while the transition matrix and free-response structure
remain fixed.
This separation lets standard linear constrained-control tools be used
with explicit assumptions: the prediction matrix $\Phi$ is precomputed,
the online realization is a 30-variable convex QP, offset-free tracking
follows from an input-centered augmented disturbance model, sampled
joint-limit safety is enforced by a one-step torque CBF when feasible,
and task-channel passivity is enforced by an energy-tank scaling layer.

The resulting controller should be viewed as one realization of the
interaction-dynamics framework rather than as a replacement for all
pHRI control modes.
In the unconstrained and disturbance-free limit it recovers the
conservative part of classical task-space impedance; under constant or
asymptotically constant bounded human force, Kalman augmentation removes
steady-state bias when the stated stability and feasibility conditions
hold; and because the transition matrix is configuration-independent, a
single Lyapunov certificate guarantees exponential stability of the
interaction error across the whole singularity-free workspace, not only at
a fixed configuration (Theorem~\ref{thm:workspace}).
MuJoCo simulation on a Franka FR3 model demonstrates a $>$800$\times$
steady-state-error reduction over classical impedance under a sustained
15\,N force, 77--97$\times$ RMSE reduction on free-space circles,
empirical joint-limit safety behavior at the workspace boundary, and
robustness to measurement noise and $\pm30\%$ inertial mismatch.
Because the interface requires only $M$, $C\dot q+G$, and $J_v$, the
same framework can be ported to other torque-controlled serial arms by
substituting the robot model and joint-limit parameters.
The next validation step is real FR3 pHRI testing with synchronized
tracking, force-estimation, CBF-residual, and energy-tank logs.

Looking further out, the value of an interaction-dynamics model with a
configuration-independent transition matrix
is not confined to impedance regulation. As embodied
systems increasingly plan through learned world models, the physical
layer---contact---remains their least data-efficient component. By reducing that
layer to a fixed linear model with a single known input matrix, the backbone
supplies an analytic dynamics prior on which a learned model need only capture
the residual uncertainty of intent, contact, and environment. We therefore see
interaction dynamics less as a single controller than as a candidate foundation
for model-based physical AI: an analytic, predictive, safety-constrained
dynamics substrate between a high-level foundation model and the robot's
torques.
