...
This objective is posed not on the robot configuration but on the interacti2n
...
After applying \eqref{eq:tff}, the residual error satisfy:
...
task-channel passivity certificate at runtime (Proposition~\ref{prop:tank}),
guaranteeing coupled stability against any passive environment.
\end{remark}
...
with vertex input matrices $B_v=[-\tfrac{\Delta t^2}{2}L_v;\,-L_v\Delta t]$. If there exist
...
This covers the online quasi-LPV build, where $\Gamma$ is frozen at the current
$B_d(q_k)$ over the horizon: the frozen-vs-future gap is a bounded variation
inside $\mathcal P$ and cannot break the decrease of $V=x^\top P x$. The frozen
horizon shapes only the \emph{predicted} trajectory, while the receding-horizon
re-solve keeps the \emph{applied} move matched to the current $\rho_k$; the
certificate carries no bounded-rate proviso.
\end{remark}
...
$2.8$--$16.9$\,kg). Over the resulting 64-vertex box ($\Delta t=10$\,ms), the
...
empirically, and is reproducible from the released script.
\end{remark}
...
Because the true disturbance $d_k$ is not directly measurable, we use the
