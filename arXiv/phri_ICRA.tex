...
baseline and stress-tested by tightening the torque budget until it
actually binds; and
...
\end{tabular}
\end{table}
\noindent $^\dagger$One-time OSQP problem setup on the episode's first tick;
excluding it, the steady-state maximum is 1.04\,ms and both rates have zero
misses. Timing excludes the separate MuJoCo dynamics query and is not
validated on an embedded or real-time-kernel target.
...
the estimator; \S V-E instead finds a genuine cost of coupling the
...
and comparing three otherwise-identical 100\,Hz controllers: \textbf{C5}
...
exceeds $\tau_{\max}$), and \textbf{Observer} (Table~\ref{tab:ablation}'s
fixed-gain baseline under the same tightened budget).
...
Unconstrained & \textbf{0.53} / 0\% & \textbf{0.53} / 0\% & \textbf{0.53} / 0\%\\
Observer & 0.52 / 0\% & 0.51 / 0\% & 232.9 / 30.9\%\\
\bottomrule
\end{tabular}
\end{table}

At the paper's operating point (scale 1.0) all three are indistinguishable,
...
550\,mm --- far worse than either alternative. Unconstrained is essentially
untouched, since the plant's physical clip alone recovers a usable command;
Observer's single-step, decoupled QP also survives this scale and only
degrades once tightened further (scale 0.2: 233\,mm, versus C5's persistent
517\,mm). This is a genuine limitation of the current all-or-nothing
infeasibility fallback under a severely tightened budget, not evidence that
embedding a torque row is unhelpful in general: at every scale tested here,
the receding-horizon-coupled feasibility requirement is more fragile than
either a single-step-decoupled QP or no embedded row at all. We do not
resolve this within the present scope; a soft/slack relaxation of
\eqref{eq:taubox} in place of the current hard zero-correction fallback is
...
consistent with the load-independent mean-equilibrium prediction rather
than a rounding artifact: Proposition~1 predicts rejection in expectation
for any constant $d$, and this near-constant $\approx$0.023\,mm empirically
matches that pattern, reflecting a numerical/discretization,
trajectory-tracking, and estimator-fluctuation floor rather than a
...
estimator under nominal actuation headroom, and \S V-E's stress test shows
the current hard, zero-correction torque-constraint fallback is a real
liability, not yet an asset, once the budget is tightened enough to bind;
a soft/slack relaxation is future work. Hardware evaluation should compare
