...
remaining, smaller contributions (inertial coupling, friction, model error)
...
state, the estimator is asymptotically unbiased for a constant $d$, and the
unconstrained first-move position gain $K_e$ is nonsingular. Then
the equilibrium satisfies $e_\infty=0$ and $F_{\mathrm{mpc},\infty}=-d$.
\end{proposition}
\begin{proof}
At equilibrium, \eqref{eq:discrete} requires
$B_d(F_{\mathrm{mpc},\infty}+d)=0$; away from singularities $B_d$ has full
column rank, so $F_{\mathrm{mpc},\infty}=-d$, and the first block row then
gives $\dot e_\infty=0$. Because $D_{\rm bar}=\Gamma(\bm 1_N\otimes I_3)$
(\eqref{eq:lifted}), $X$ depends on $U$ only through $V=U+d_N$, so the cost
\eqref{eq:cost} written in $V$ is independent of $\hat d$ and the
unconstrained law is exactly $F_{\mathrm{mpc},0}=K_ee+K_{\dot e}\dot e-\hat d$
for every $\hat d$, not only $\hat d=0$. At equilibrium, with
$\hat d_\infty=d$ by unbiasedness, $F_{\mathrm{mpc},\infty}=K_ee_\infty-d$.
Equating the two expressions for $F_{\mathrm{mpc},\infty}$ gives
$K_ee_\infty=0$, hence $e_\infty=0$ since $K_e$ is nonsingular.
\end{proof}
...
recursive filter's predict step in \eqref{eq:kf} uses this QP-solved force
...
Proposition~1 predicts exact rejection for any constant $d$, so 0.021\,mm
reflects a load-independent numerical/discretization and trajectory-tracking
floor, not a residual scaling with the applied force.
