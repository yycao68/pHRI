...
model cascaded with low-level PD control; here the disturbance is treated as
unknown rather than explicitly predicted, and the correction is realized
through a single affine constraint on the complete applied joint torque
rather than a cascaded loop. The present contribution is therefore not online
force estimation or predictive interaction control as such, but their
combination into a sensorless, constant-transition offset-free interface with
a proved frozen-configuration equilibrium and a first-move
torque-feasibility guarantee.
