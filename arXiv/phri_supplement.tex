...
\title{Supplementary Material: Toward Interaction Dynamics --- A Predictive
Framework for Safe Physical Human–Robot Interaction}
...
% ----------------------------------------------------------
\section{Proof of Theorem~3 --- Quadratic Stabilizability of the Interaction-Dynamics Backbone}\label{sec:stab}

\begin{theorem}[Quadratic Stabilizability of the Interaction-Dynamics
Backbone]\label{thm:workspace}
...
$Y\in\mathbb R^{3\times6}$ such that, for every vertex $v=1,\dots,V$,
\begin{equation}
\begin{bmatrix}Q & (A_dQ+B_vY)^\top\\A_dQ+B_vY & Q\end{bmatrix}\succ0,
\label{eq:lmi}
\end{equation}
then the \emph{fixed} state-feedback law $\Fmpc=Kx_e$ with $K=YQ^{-1}$
makes $V(x)=x^\top Px$, $P=Q^{-1}$, a \emph{common} quadratic Lyapunov
function: $V(x_{e,k+1})\le\gamma\,V(x_{e,k})$ with $\gamma<1$ for
...
\eqref{eq:lmi} is affine in this quantity, and the positive-semidefinite
cone is convex, so \eqref{eq:lmi} holds for all $\rho\in\mathcal P$, not
only at the vertices. A Schur complement on \eqref{eq:lmi} gives
$(A_dQ+B_d(\rho)Y)\,Q^{-1}(A_dQ+B_d(\rho)Y)^\top\prec Q$; writing
$A_\text{cl}(\rho)=A_d+B_d(\rho)K$ with $K=YQ^{-1}$ yields
$A_\text{cl}(\rho)QA_\text{cl}(\rho)^\top\prec Q$, i.e.\
$A_\text{cl}(\rho)^\top PA_\text{cl}(\rho)-P\prec0$ with $P=Q^{-1}$, for
...
interaction-dynamics \emph{representation} --- enabled by the constant
...
safety and passivity filters (Theorem~4, Proposition~2 of the main
...
floor). This instantiates $\mathcal P$ over the \emph{sampled operating
...
at every vertex, \emph{for the certified fixed $K=YQ^{-1}$} --- the
...
(\S V) and this supplement is consolidated below, cross-checked against
the released code (\texttt{simulation/impedance\_mpc.py},
\texttt{simulation/phri.py},
\texttt{cloud\_verify/lib/fr3\_hardware\_interface.py}) rather than
...
\begin{tabular}{p{3.1cm}p{0.9cm}p{2.0cm}}
\toprule
...
FR3 torque limits & $\tau_\text{max}$ & $[87,87,87,87,12,12,12]$\,Nm\\
...
OSQP tolerances & $\epsilon_\text{abs},\epsilon_\text{rel}$ & $10^{-6}$\\
...
FR3 neutral pose & $q_0$ & $[0,-0.785,0,-2.356,0,1.571,0.785]$ rad\\
\bottomrule
\end{tabular}
\end{table}
...
$^\dagger$Benchmark I (Table~\ref{tab:bm1}) and Benchmark II
(Table~\ref{tab:bm2}) are run with $k_\text{ws}=0$: the driver scripts
...
test (Table~\ref{tab:safety}) actually uses, which is where the
...
\begin{equation*}
-\tau_\text{max}\le\tau_\text{base}(0)+J_v^\top(q_0)\bigl(F_\text{bb}+\Fmpck{i}\bigr)\le\tau_\text{max},\ \ i=0,\ldots,N-1,
\end{equation*}
...
bounded rather than diverging. The offset-free/stability statements of
Theorems~1--3 of the main paper are proved for the standard
