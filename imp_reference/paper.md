...
\(\bar N_v(q)\) is a dynamically consistent projector for the translation task [9]. An earlier implementation projected the null-space term with the full 6-row Jacobian and left the orientation term unprojected; both leaked into \(\ddot e\), so joint drift increased auxiliary torque and then the compensating Cartesian command. Projecting both terms removes that structural path. Because \(\Lambda\) uses Tikhonov regularization, \(J_vM^{-1}\bar N_v^\top=0\) holds only approximately; the remaining leakage and \(\dot J_v\dot q\) are therefore retained in \(d_{\mathrm{known}}\).
...
The 2 s hold gives the admittance velocity time to approach steady state (\(T_a=0.3\) s). With the earlier 0.5 s hold, the reactive comparator did not approach the workspace bound, so that case offered no predictive-versus-reactive contrast for admittance. The impedance peak is determined by its equilibrium and is reached within 0.5 s at either hold length.
...
The same pattern applies to model-based physical AI: an upstream learned or language-conditioned component could propose behavior while a realization runtime evaluates feasibility and exposes any intervention. This paper validates only the downstream interface for the affine subclass; a future physics-based, optimization-based, or learned layer needs only to expose a compatible realization contract, while stateful or uncertain interfaces remain a research direction, not a validated instance of the present QP.
