...
The deployed QP uses the exact slack reparameterization \(z=10^3s\), reducing the condensed Hessian condition number from about \(4\times10^9\) to \(4\times10^3\) without changing the physical objective or feasible set; a matrix-level unit test checks the equivalence. A dedicated timing study (`simulation/run_fr3_timing_study.py`, 5 full 6 s benchmark repetitions and 1500 actual solves per generator/condition) measures cold and warm starts separately. With the deployed warm start, impedance records 3.45 ms mean, 6.89 ms p99, and 8.67 ms maximum; admittance records 3.68, 7.28, and 9.73 ms. All 3000 warm solves meet the 20 ms manager period on the development machine. The scaled cold-start ablation also meets the period in the saved run (17.73/14.92 ms maxima), showing that conditioning—not claim wording—closed the earlier tail-latency problem. These are machine-specific measurements, not a hard real-time guarantee; the synchronous simulator does not implement asynchronous stale-result discard.
...
|---|---:|---:|---:|---:|
| All 15 predicted steps | 0.0002 | 0.161 | 1.398 | 0.0603 |
| First predicted step only | 11.329 | 0.380 | 1.466 | 0.0619 |
