...
The framework is one realization, not a replacement for all pHRI modes: for intentional co-manipulation (guiding the robot by hand) admittance control remains the appropriate paradigm, and near the workspace boundary joint-limit regulation needs the task-space projection (13) alongside the null-space barrier. The next validation step is real FR3 pHRI testing with synchronized tracking, force-estimation, CBF-residual, and energy-tank logs.
