...
### 7.4 Anticipatory disturbance forecasting: a foresight/reactivity trade-off

The main benchmark's disturbance term (Section 6.1) contains three sinusoids of known, fixed frequency (0.9, 1.4, 1.9 Hz). This raises a natural question: if those frequencies are known in advance -- a disclosed, benchmark-specific modeling choice, not a general online frequency-identification claim -- can the MPC do better than holding its disturbance estimate constant across the horizon (the frozen zero-order hold used everywhere else in this paper)? An **anticipatory** variant tracks each axis with a small per-axis Kalman filter over the two-dimensional sine/cosine state at the known frequency and forecasts that fitted model forward through the full 0.20 s horizon, replacing the frozen hold on \(\hat d_k\) in equation (11) with a per-step forecast; everything else -- the same tank, the same torque authorization, the same measurement -- is unchanged.

Two wall-disabled, twenty-seed scenarios isolate the comparison from contact: **periodic-only** (all three sinusoids plus the pulse active, the direct test of whether forecasting the known-frequency component helps) and **pulse-only** (sinusoids silenced, the 12 N pulse alone active, the negative-control case where a harmonic model has nothing periodic to lock onto). A third, satellite check reruns the full matched benchmark (wall active, everything on) to see whether any advantage survives once contact-force content dominates the disturbance estimate.

**Table 6. Anticipatory (harmonic forecast) vs. two-rate (frozen hold), paired by seed.**

| Scenario | two-rate RMS (mm) | anticipatory RMS (mm) | paired difference (mm) | 95% CI | \(p\) |
|---|---:|---:|---:|---:|---:|
| Periodic-only | **20.409 ± 1.679** | 20.737 ± 1.628 | +0.328 | [0.220, 0.436] | \(4.24\times10^{-6}\) |
...
\(^\dagger\)With the sinusoids silenced (\(\text{disturbance\_scale}=0\)), the per-seed random phases no longer reach the trial; without another source of per-seed variation, all 20 seeds would replay an identical trajectory and any paired statistic here would be spurious. This scenario adds the same 0.05 N estimator noise used by the leakage and sensing-realism sweeps (Section 6.3) as its sole source of seed-to-seed variation, giving the 20 seeds genuine independence.

The headline result is the opposite of what motivated the study: in periodic-only, the scenario built specifically to let forecasting help, the anticipatory controller is 1.6% *worse* than the frozen hold, not better. This is not a prediction-accuracy failure -- fed the same noiseless sinusoid the simulator generates, the per-axis Kalman filter's forecast error settles to the order of \(10^{-6}\) mm within 0.5 s, and a full sweep of a blend parameter that linearly interpolates between the frozen hold (\(0\)) and the full forecast (\(1\)) degrades RMS *monotonically* -- 20.41, 20.48, 20.56, 20.65, 20.74 mm at blend \(0, 0.25, 0.5, 0.75, 1\) -- ruling out a threshold or instability effect. Directly inspecting the QP at a disturbance peak explains the mechanism: given an accurate forecast that the disturbance is about to swing back toward zero, the optimizer commits *less* immediate corrective wrench than the frozen-hold controller, which incorrectly assumes the disturbance persists at its current value for the full horizon and therefore always corrects harder right now. Because the horizon's stage cost (equation (12)) weights every step \(\xi_i^\top Q\xi_i\) equally, with no discount or terminal emphasis on the near term, the MPC has no structural reason to prefer the immediate error the RMS metric actually measures over the horizon-averaged cost it is actually minimizing. An accurate forecast of a genuinely periodic disturbance makes the controller *more* correct about the future and *less* reactive right now -- and this benchmark's metric rewards the latter, not the former.

A natural objection is that 0.20 s (\(N=20\)) simply is not enough lookahead -- perhaps a longer horizon lets the controller collect the benefit of foresight instead of only its cost. An eight-seed horizon sweep (`horizon_sweep.py`, \(N\in\{10,20,40,60,80,110\}\), i.e. 0.10-1.10 s -- past a full period of the slowest, 0.9 Hz sinusoid) rules this out for periodic-only:
...
| 20 (0.20 s, paper default) | 20.739 | 21.156 | +2.0% |
| 40 (0.40 s) | 20.091 | 20.868 | +3.9% |
| 60 (0.60 s) | 20.081 | 20.879 | +4.0% |
| 80 (0.80 s) | 20.085 | 20.847 | +3.8% |
| 110 (1.10 s) | 20.094 | 20.840 | +3.7% |

The gap widens, not narrows, with lookahead -- from 0.3% at 0.10 s to 3.9% by 0.40 s -- then plateaus near 3.7-4.0% out to 1.10 s. More lookahead gives the optimizer more, not less, opportunity to anticipate the disturbance's own future self-cancellation and correct less aggressively now; it saturates once the horizon covers enough of a cycle that additional length adds no new exploitable information. A longer horizon is not the fix.
...
At \(N=10\) and \(N=20\), anticipatory is modestly better (-0.5%, -1.8% -- the latter consistent with Table 6's -1.9% at \(N=20\) within this smaller sample's noise); at \(N=40\) it reverses to a 9.5% disadvantage and stays substantially worse (7.6-12.6%) through \(N=110\). **The full-matched advantage reported in Table 6 is therefore a narrow, horizon-specific artifact of the paper's own \(N=20\) design point, not a robust benefit of forecasting under contact.** An instrumented, single-seed trace of the predictor's own fitted state explains why a longer horizon makes this worse rather than better: contact drives the filter's error against the true periodic component from 0.037 mm (periodic-only, uncontaminated) up to 4.56 mm during contact, and the estimate has not recovered even 0.30 s after contact ends (0.929 mm, still roughly \(30\times\) the uncontaminated case) -- a persistent, not merely instantaneous, corruption of exactly the forecast a longer horizon leans on more heavily. The pulse-only row (Table 6) is a genuine negative control -- the harmonic model has no periodic content to lock onto -- and it, too, is significantly worse for the anticipatory controller, consistent with the periodic-only finding rather than merely directionally similar to it.

### 7.5 Interpretation

The benchmark supports three claims. First, finite-horizon residual correction can improve the realized intentional impedance response under disturbances. Second, strict zero-reference PO/PC is substantially more conservative than a finite tank that can spend initial and dissipated energy. Third, the fast layer can preserve its explicit tank and torque interfaces even when the force classifier is wrong, so those certificates must not be confused with task or intent correctness. A fourth, unplanned finding (Section 7.3) is that no single manager rate is best across scenario types -- the right rate depends on whether the disturbance is oscillatory or a sustained step, and the paper's own headline rate is a choice for one of those regimes, not a universal optimum. A fifth, also unplanned finding (Section 7.4) is that accurate disturbance foresight is not unconditionally beneficial under this benchmark's flat, undiscounted horizon cost: forecasting a genuinely periodic disturbance made the controller *less* effective at the instantaneous tracking metric this paper reports, because the optimizer correctly, and unhelpfully, anticipates the disturbance's own future self-cancellation -- a gap that widens, not narrows, with a longer horizon. The one condition where forecasting appeared to help (wall contact present) does not survive that same horizon check either, so no configuration tested in this paper supports disturbance forecasting as a robust improvement over the frozen hold.
