...
Residual model predictive control (MPC) can improve disturbance rejection around a compliant robot, but its corrective wrench is an active interaction port. A passivity check performed only when the MPC updates does not authorize the wrench held between updates, and strict time-domain passivity control can remove most of the predictive correction. We study a rate-separated architecture in which a motion-to-wrench impedance remains the physical nominal controller, a 100 Hz MPC proposes only a translational residual wrench, and a 1 kHz projection jointly enforces a residual-energy budget and the complete joint-torque envelope. This is not presented as the first combination of impedance, MPC, and passivity: passive model-predictive impedance, predictive variable-impedance/admittance, and energy-tank controllers are established. The investigated contribution is the explicit residual-port authorization and its inter-update realization, certified by a continuous-time composition proposition and a fast-sample energy-floor lemma, and benchmarked against an equation-faithful translational generalization of the Hannaford--Ryu time-domain passivity observer/controller. In a torque-controlled 7-DoF Franka FR3 MuJoCo benchmark, 20 matched trials randomize passive-wall properties, disturbance amplitude, and phase; the proposed method has zero tank-floor violations, zero nominal-torque infeasibilities, and zero QP failures, reducing residual-position RMS by 23.7% relative to passive impedance and by 35.4% relative to strict time-domain passivity control, while remaining 19.5% worse than energetically unguarded MPC. The central result isolates why fast authorization is necessary: a manager-rate-only guard sharing the identical authorization rule but re-checked once per 10 ms manager period instead of every 1 ms fast tick tracks statistically indistinguishably from the proposed controller (\(p=0.656\)) yet breaches the tank floor in 20/20 trials, against zero violations in 20/20 for fast-sample re-authorization -- periodic checking at the update rate is not a substitute for authorization refreshed every fast sample between updates. A force-separation check further shows that energy and torque correctness do not imply correct interaction semantics: the wall's own reaction force is folded into the same disturbance estimate by construction, so the controller commands a corrective push into physical contact with zero energy-tank engagement, a classification failure the tank certificate cannot see. The results support the two-rate authorization mechanism in full rigid-body simulation, but not hardware-level safety or universal superiority.
...
**Fast loop, at the robot's own servo rate, 1 kHz.** The physical nominal controller is an impedance law: it looks at the current position and velocity error and computes a wrench directly, the way a stiff spring-damper would. This wrench is converted to joint torque and, added to it, the *residual* wrench the slow loop most recently published (converted to torque at the *current* configuration, not cached from when it was computed). The residual wrench is then scaled against the instantaneous joint-torque headroom and available tank energy before being added to the feasible nominal torque, and only then sent to the robot. This last step is the fast authorization; it runs every 1 kHz tick regardless of what the slow loop is doing.

**Slow loop, every manager period, 100 Hz -- ten times slower than the fast loop, not ten times a second.** The manager does not touch the impedance law. Instead, every period, it solves a short-horizon optimization: predict the impedance law's own error forward over the manager's look-ahead (Section 5.1 fixes this at 0.20 s, 20 manager steps) and propose the additive residual wrench that best corrects that predicted error, subject to the joint-torque envelope. This correction is generically nonzero whenever the predicted error is nonzero -- it is a tracking proposal computed every period, not a fallback triggered only when something is about to go wrong. The manager's own optimization does not see the energy budget at all; keeping the proposed wrench within that budget once it reaches the robot is entirely the fast loop's job, described next.
...
*in words:* storage grows from external work \(F^\top v\), shrinks from the impedance law's own damping \(-v^\top D_Iv\), and the last term, \(F_r^\top v\), is the power the residual wrench injects or removes — this is the port that Section 5.2's tank must authorize, because nothing about the impedance law itself limits it. Equation (7) is exact under the constant-\(\Lambda\) idealization used in Proposition 1. For the configuration-varying robot, differentiating (6) introduces an additional \(\tfrac12v^\top\dot\Lambda v\) term that (7) does not carry; the paper therefore does not use (7) as an exact storage identity for the implemented nonlinear plant. The guarantee actually implemented is instead the fast-sample ledger and torque invariant of Lemma 1.
...
The benchmark supports three central conclusions. First, a finite, replenishable energy tank recovers most of the tracking that strict zero-reference PO/PC removes: finite-horizon residual correction improves the realized intentional impedance response under disturbances, and spending initial and dissipated energy is substantially less conservative than zero-reference damping. Second, that residual authority must be refreshed at the fast realization rate specifically, not merely exist: the B4 manager-rate guard shares B5's MPC proposal and authorization rule exactly, tracks statistically indistinguishably from it, and still breaches the tank floor in every trial once \(\alpha_E\) is held stale for even one manager period -- periodic re-authorization is not a cheaper substitute for fast-sample inter-update authorization, it is a different, weaker guarantee. Third, energy and torque correctness do not imply semantic correctness: the fast layer can preserve its explicit tank and torque interfaces even when the force classifier is wrong, so those certificates must not be confused with task or intent correctness. Leakage causes under-response to genuine human input with the certificates intact, and, more severely, folding the wall's own reaction into the same disturbance estimate causes the controller to push into physical contact with zero tank engagement, since the failure is a classification error the energy and torque certificates were never designed to catch.

Two additional observations are secondary to these three. Manager-rate sensitivity (Section 7.3): no single proposal rate is best across scenario types -- the right rate depends on whether the disturbance is oscillatory or a sustained step, and the paper's own headline rate is a choice for one of those regimes, not a universal optimum. Anticipatory forecasting (Section 7.4, Appendix A): a harmonic disturbance forecaster only matches the frozen hold when the disturbance is genuinely harmonic, and loses to it -- more so with a longer horizon -- once any non-harmonic content is present; the one condition where it appeared to win reverses under the same check, so no configuration tested here supports forecasting as a robust improvement.
