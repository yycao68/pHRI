...
- **B3\('\) Manager-rate guard:** the same raw MPC and (17)'s energy scale \(\alpha_E\), but \(\alpha_E\) is computed once per manager tick and held fixed across the ten fast ticks it spans, rather than recomputed every fast tick as in B4; isolates whether authorization must run at the fast rate specifically, not merely whether some energy authorization exists;
- **B4 Two-rate tank:** the same raw MPC plus (16)--(19), recomputing \(\alpha_E\) every fast tick.
...
| Manager-rate guard (B3\('\)) | 19.40 ± 0.58 | 31.01 ± 1.59 | 0.0184 ± 0.0004 | 25.34 ± 3.91% | 0.910 ± 0.017 |
...
**PO/PC's degraded RMS is not simply the cost of a stricter passivity philosophy.** An instrumented six-seed check finds \(\gamma_\ell\) (equation (21)) strongly dependent on port speed: the Pearson correlation between \(1/\|v_\ell\|\) and \(\gamma_\ell\) is 0.94--0.98 across seeds, mean \(\gamma_\ell\) is 40--60\(\times\) larger when \(\|v_\ell\|<5\) mm/s than otherwise -- a regime occupied 38--53% of each trial -- and per-tick gains reach \(10^4\)-\(10^5\). Equation (21)'s velocity-squared denominator, inherited unmodified from [7], is therefore a material and specifically diagnosed contributor to PO/PC's RMS exceeding even passive impedance's (30.00 vs. 25.42 mm), not only its zero-reference conservatism in the abstract. A bounded-gain or dead-zone variant of the same passivity principle would likely change this comparison, but is not substituted here, since the baseline is deliberately equation-faithful to the published formula rather than a tuned competitor.

**The manager-rate guard directly tests the title's claim.** B3\('\) shares B4's MPC proposal and authorization rule exactly, differing only in how often \(\alpha_E\) is recomputed: every fast tick for B4, once per manager tick and held stale across the following nine for B3\('\). Its tracking is statistically indistinguishable from B4's (paired difference +0.008 mm, 95% CI [-0.029, 0.045], \(p=0.656\)) -- fast re-authorization costs nothing in tracking quality -- yet its tank floor is breached in every one of 20 trials (mean violation 0.0016 J, range 0.0010-0.0022 J), against zero violations in 20/20 for B4. A staleness of at most one manager period (10 ms) is therefore sufficient to break the floor guarantee at this benchmark's dynamics, even though the held \(\alpha_E\) was itself computed correctly at the instant it was set: the guarantee requires re-checking, not merely checking. Between-update authorization is not a redundant safety margin; it is what the guarantee actually rests on.
