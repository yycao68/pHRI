...
The central construct is therefore a *behavior--realization interface*: a fixed contract, at the level of a requested task acceleration, that any fast controller can expose and any robot-specific realization map can consume. The two-rate architecture, the certificate-transfer theorem, and the directional-authority indicator are consequences of adopting that interface, not independent ideas bundled alongside it.
...
2. robot-specific feasible acceleration geometry, together with a directional-authority indicator logged at every predictive update, exposing failures hidden by scalar torque utilization;
3. a conditional certificate-transfer theorem that separates a reusable certified action set from robot-specific actuator and successor-error tests, together with a minimal constructive instantiation of that set --- chosen to demonstrate QP-enforceability rather than to be the definitive form of the certificate --- enforced as a QP constraint rather than checked after the fact;
...
This affine-image view is a geometric interpretation only; neither the optimizer nor the directional-authority metric of Section IV enumerates its facets or vertices. Both instead act on the halfspace representation \(\mathcal A_r^{\mathrm{tight}}(x)\) obtained after tightening (Section IV), for which membership and the directional query below reduce to linear inequalities.
...
**Remark (minimal constructive instantiation).** The QP of Section IV enforces \(a_{\mathrm{req},i|\ell}\in\mathcal A_r^{\mathrm{tight}}(\hat x^0_{i|\ell})\cap\mathcal K_{\mathrm{cert}}(z_{i|\ell})\), so that any feasible solution provably satisfies both sets by construction rather than by a check applied after the fact. We intentionally choose the simplest nonempty constructive instance of \(\mathcal K_{\mathrm{cert}}\), in order to demonstrate that the transfer theorem can be instantiated inside the QP without requiring nonlinear optimization; this instantiation is a proof-of-concept existence witness, not offered as the definitive or most informative certified action set for this problem. Let
...
The limitations below share a single theme --- what is verified along sampled trajectories versus what would need to hold over a continuous workspace or an unbounded horizon --- rather than being independent concerns:

- **Model fidelity.** The robot substitutions are reduced-order actuator-geometry surrogates, not full rigid-body or hardware systems, and the reported errors are trajectory observations rather than certified bounds over a continuous workspace.
- **Scope of the instantiated certificate.** Only the abstract side of Theorem 1's premise is concretely instantiated: \(\mathcal S\) and \(\mathcal K_{\mathrm{cert}}\) are enforced, using an \(\infty\)-norm storage function tightened on velocity alone for one 2-D reduced-order model, but the physical containment \(\mathcal D_{z,r}\subseteq\mathcal E_\star\) that robust invariance also requires remains a sampled audit, not a workspace-wide proof.
- **Excluded physical effects.** Orientation, redundant null-space tasks, sensor delay, contact transitions, state-estimation uncertainty, human-participant validation, and the task-channel dissipativity sketch of Section V are outside the benchmark.
- **Baseline scope.** The reference-governor baseline is an architecture-level comparator, not a reproduction of every established governor design; its scalar command parameterization is especially restrictive in the directional-collapse case, and a directional or vector governor would be expected to narrow the reported lead-time gap.
- **Statistical scope.** Every reported case is a single deterministic trajectory per scenario, controller, and realization map, so the reported margins are point estimates rather than statistically characterized safety margins across disturbance magnitude, timing, or sensor noise.
- **Horizon scope.** Region persistence and recursive feasibility are the same unresolved gap at different levels: a formally constructed terminal invariant set or backup policy would be needed before finite-horizon running constraints imply global recoverability.

---

# VIII. Conclusion

This paper introduced a behavior--realization interface for actuator-limited fast robot controllers: a fixed contract, at the level of a requested task acceleration, that lets a two-rate predictive architecture, a certificate-transfer theorem, and a directional-authority indicator all be stated once and reused across controllers and robots, rather than three separate mechanisms. The nominal controller remains at \(1~\mathrm{kHz}\), while a slower MPC layer forecasts robot-specific loss of realizability and modifies the requested acceleration before clipping. The proposed separation does not make actuator feasibility universal. It identifies which theoretical object may be reused—a certified action set stated in behavior coordinates—and which objects must be verified again—the realization map, actuator margin, uncertainty bounds, and operating region. A minimal constructive instantiation of that certified action set is enforced inside the QP itself, so the theorem's key hypothesis holds by construction rather than by a check applied afterward; it is offered as an existence witness for QP-enforceability, not as the definitive form the certificate should take. For model-based physical AI in particular, this interface is the runtime boundary: learned or language-conditioned modules may propose behavior at the same acceleration-request contract, while the realization layer evaluates what the current robot can execute, without certifying the semantics of the proposed command.
