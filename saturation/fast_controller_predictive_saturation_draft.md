...
The central construct answering this question is a *behavior--realization interface*: a fixed contract, at the level of a requested task acceleration, that any fast controller can expose and any robot-specific realization map can consume. The two-rate architecture, the certificate-transfer theorem, and the directional-authority metric below are consequences of adopting that interface, not independent ideas bundled alongside it.
...
3. Robot-specific feasible acceleration geometry: an affine image of the actuator box, interpretable as a zonotope, that represents actuator-limited task acceleration in a configuration-dependent way.
...
This affine-image view is a geometric interpretation only; neither the optimizer nor the directional-authority metric of Section 5.2 enumerates its facets or vertices. Both instead act on the halfspace representation \(\mathcal A_r^{\mathrm{tight}}(x)\) obtained after tightening (Section 6.3), for which membership and the directional query reduce to linear inequalities.
...
This paper introduced a behavior--realization interface: a fixed contract, at the level of a requested task acceleration, that lets a two-rate architecture, a certificate-transfer theorem, and a directional-authority metric all be stated once and reused across controllers and robots, rather than three separate mechanisms. It does not propose MPC as a replacement for fast robot control. PD, impedance, RL, neural, and AI-conditioned controllers remain in the \(1~\mathrm{kHz}\) loop. A slower predictive realization manager forecasts when their requested dynamics will become unrealizable under actuator saturation and introduces the smallest behavior correction before direct clipping is required. For model-based physical AI in particular, this interface is the runtime boundary between behavior generation and physical realization (Section 13.5).

The double-integrator prediction backbone is useful but classical. The contribution is the separation that permits a behavior-level certificate to be reused through an explicit robot-specific refinement condition, and a certified action set stated in behavior coordinates. A minimal constructive instantiation of that certified action set is enforced inside the QP itself, so the theorem's key hypothesis holds by construction rather than by a check applied afterward; it is offered as an existence witness for QP-enforceability, not as the definitive form the certificate should take. In 111 deterministic cases, the interface accepted five fast-controller types, one empirical successor-defect budget passed sampled checks across three realization surrogates, and a dedicated ablation confirmed the certified action set changes the manager's output when it binds. Only the abstract side of the theorem's premise is instantiated this way; the physical containment the robust-invariance premise also requires remains a sampled audit, not a workspace-wide proof. Horizon-wide correction prevented the slow, directional, and near-boundary braking violations, while abrupt disturbance and severe mismatch provided explicit counterexamples outside the theorem's premises.
