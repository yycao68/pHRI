...
Status: **folded into the manuscript as a robustness ablation (§VI-I / Table VIII, both `double_integrator_phri_ieee.md` and `arXiv/phri_main.tex`, compiled clean).** This documents the architectural change (a fixed impedance backbone commanded independently of the QP, §2), its implementation as a new controller option (as **C8** in the paper's numbering — the manuscript's existing Table already used C6 for a different controller), and the simulation comparison against D7 that motivated adding it, including a protocol-matched 3-cycle re-run (§4.3) confirming the 1-cycle snapshot generalizes. **§7 (horizon scheduling) is a settled negative result, kept OUT of the paper: §2c's frozen-Jacobian formulation is what shipped.** A constant-velocity coast extrapolation was tried and removed (unjustifiable second approximation, no benefit); reference scheduling along the trajectory generator's own $(q_d,\dot q_d,\ddot q_d)$ — the theoretically cleaner alternative — was then implemented and found to be not just unnecessary but a measurable **regression** (up to 26× worse steady-state error) in the regime where the torque constraint binds, because it schedules against the undisturbed reference exactly when the real robot has deflected away from it during contact. Reference-scheduling code remains available for a future longer-horizon/slower-rate revision but is not recommended now, and was not folded into the manuscript.
...
1. **Resolved — folded into the manuscript.** C6 (now **C8** in the paper's own numbering — the manuscript's Table "Benchmarked Controllers" already used C6 for a different controller, `DI-MPC, 500 Hz, no Kalman`) was added as §VI-I, "Correction-Authority Robustness (Impedance-Backbone Ablation)," in both `double_integrator_phri_ieee.md` and `arXiv/phri_main.tex` (Table VIII + figure, compiled clean, 12 pp — up from the prior 11 pp trim target, worth a look). Added as a robustness ablation alongside D7, not a replacement — consistent with the recommendation here. Not re-derived: Theorems 1–3 for the backbone-in-the-loop case (explicitly scoped as future work in the new §VI-I text).
